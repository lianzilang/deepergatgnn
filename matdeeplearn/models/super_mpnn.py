...
# from torch_geometric.nn import Set2Set, global_mean_pool, global_add_pool, global_max_pool, NNConv, DiffGroupNorm
# from torch_scatter import scatter_mean, scatter_add, scatter_max, scatter


class SUPER_MPNN(paddle.nn.Layer):

    def __init__(self, data, dim1=64, dim2=64, dim3=64, pre_fc_count=1,
        gc_count=3, post_fc_count=1, pool='global_mean_pool', pool_order=
        'early', batch_norm='True', batch_track_stats='True', act='relu',
        ...
        self.gru_list = paddle.nn.LayerList()
        self.bn_list = paddle.nn.LayerList()
        for i in range(gc_count):
            nn = paddle.nn.Sequential(paddle.nn.Linear(in_features=data.
                num_edge_features, out_features=dim3), paddle.nn.ReLU(),
                paddle.nn.Linear(in_features=dim3, out_features=gc_dim *
                gc_dim))
            conv = NNConv(gc_dim, gc_dim, nn, aggr='mean')
            self.conv_list.append(conv)
            gru = paddle.nn.GRU(input_size=gc_dim, hidden_size=gc_dim,
                time_major=not False)
            self.gru_list.append(gru)
            if self.batch_norm == 'True':
                bn = DiffGroupNorm(gc_dim, 10, track_running_stats=self.
                    batch_track_stats)
    ...
    def forward(self, data):
        ...
        if len(self.pre_lin_list) == 0:
            ...
            h = out.unsqueeze(axis=0)
        for i in range(0, len(self.conv_list)):
            if len(self.pre_lin_list) == 0 and i == 0:
                if self.batch_norm == 'True':
                    m = self.conv_list[i](data.x, data.edge_index, data.
                        edge_attr)
                    m = self.bn_list[i](m)
                else:
                    m = self.conv_list[i](data.x, data.edge_index, data.
            ...
            m = paddle.nn.functional.dropout(x=m, p=self.dropout_rate,
                training=self.training)
            out, h = self.gru_list[i](m.unsqueeze(axis=0), h)
