      ...
      reactions(pubId: "{pub_id}", first: {page_size}, after: "{start_cursor}") {{
        ...
        edges {{
          node {{
            ...
            .format(start_cursor=start_cursor, page_size=page_size, pub_id=
            pub_id))
        has_next_page = data['reactions']['pageInfo']['hasNextPage']
        start_cursor = data['reactions']['pageInfo']['endCursor']
        page += 1
        print(has_next_page, start_cursor, page_size * page, data[
    ...
    for i, reaction in enumerate(reactions):
        for j, _ in enumerate(reactions[i]['reactionSystems']):
            ...
            calculator = ase.calculators.singlepoint.SinglePointCalculator(
                atoms, energy=system.pop('energy'))
            atoms.set_calculator(calculator)
            reactions[i]['reactionSystems'][j]['atoms'] = atoms
        reactions[i]['reactionSystems'] = {x['name']: x['atoms'] for x in
...
print(len(reactions))
equations = ['0.5H2(g) + * -> H*', 'H2O(g) - H2(g) + * -> O*',
    'H2O(g) - 0.5H2(g) + * -> OH*', 'H2O(g) + * -> H2O*',
    'CH4(g) - 2.0H2(g) + * -> C*', 'CH4(g) - 1.5H2(g) + * -> CH*',
    'CH4(g) - H2(g) + * -> CH2*', 'CH4(g) - 0.5H2(g) + * -> CH3*',
    '0.5N2(g) + * -> N*', '0.5H2(g) + 0.5N2(g) + * -> NH*',
    'H2S(g) - H2(g) + * -> S*', 'H2S(g) - 0.5H2(g) + * -> SH*']
key_name = ['Hstar', 'Ostar', 'OHstar', 'H2Ostar', 'Cstar', 'CHstar',
    'CH2star', 'CH3star', 'Nstar', 'NHstar', 'Sstar', 'SHstar']
        ...
        try:
            if reactions[j]['Equation'].find(equations[i]) != -1:
                if reactions[j]['sites'].find('top|') != -1:
                    ase.io.write('ads_data/' + str(count) + '.json',
                        reactions[j]['reactionSystems'][key_name[i]])
                    count = count + 1
