...
from torch_geometric.nn import Set2Set, global_mean_pool, global_add_pool, global_max_pool, DiffGroupNorm
from torch_scatter import scatter_mean, scatter_add, scatter_max, scatter
from torch_geometric.nn.models.schnet import InteractionBlock


class SUPER_SchNet(paddle.nn.Layer):

    def __init__(self, data, dim1=64, dim2=64, dim3=64, cutoff=8,
        pre_fc_count=1, gc_count=3, post_fc_count=1, pool=
        'global_mean_pool', pool_order='early', batch_norm='True',
        batch_track_stats='True', act='relu', dropout_rate=0.0, **kwargs):
        ...
        for i in range(gc_count):
            conv = InteractionBlock(gc_dim, data.num_edge_features, dim3,
                cutoff)
    ...
    def forward(self, data):
        ...
        for i in range(0, len(self.conv_list)):
            if len(self.pre_lin_list) == 0 and i == 0:
                if self.batch_norm == 'True':
                    out = data.x + self.conv_list[i](data.x, data.
                        edge_index, data.edge_weight, data.edge_attr)
                    out = self.bn_list[i](out)
                else:
                    out = data.x + self.conv_list[i](data.x, data.
                        edge_index, data.edge_weight, data.edge_attr)
            elif self.batch_norm == 'True':
                out = out + self.conv_list[i](out, data.edge_index, data.
